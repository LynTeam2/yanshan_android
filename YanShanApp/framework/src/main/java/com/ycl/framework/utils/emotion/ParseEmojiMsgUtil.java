...
public class ParseEmojiMsgUtil {
    ...
    public static void dealExpression(Context context, SpannableString spannableString,
        ...
        while (matcher.find()) {
            ...
            bitmap = EmotionLruCache.getInstance().getEmoteFromCache(key);
            if (bitmap == null) {
                try {
                    ...
                    if (resId != 0) {
                    	...
                    	bitmap = Bitmap.createBitmap(bitmaps, 0, 0,
                                bitmaps.getWidth(), bitmaps.getHeight(), matrix,
                                true);
                    	bitmaps.recycle();
                    }
                } catch (Exception e) {
                    bitmap = BitmapFactory.decodeResource(context.getResources(),
                            R.drawable.emoji_316);
                }
                EmotionLruCache.getInstance().putEmoteToCache(key, bitmap);
            }
